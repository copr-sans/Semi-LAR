...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        if mask is not None:
            ...
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N * ratio) + mask.unsqueeze(
                1
            ).unsqueeze(0)
...
class LeFF(nn.Module):
    def __init__(self, dim=32, hidden_dim=128, act_layer=nn.GELU, drop=0.0):
        ...
        self.dwconv = nn.Sequential(
            nn.Conv2d(
                hidden_dim, hidden_dim, groups=hidden_dim, kernel_size=3, stride=1, padding=1
            ),
            act_layer(),
        )
...
class OutputProj(nn.Module):
    def __init__(
        self, in_channel=64, out_channel=3, kernel_size=3, stride=1, norm_layer=None, act_layer=None
    ):
        super().__init__()
        self.proj = nn.Sequential(
            nn.Conv2d(
...
if __name__ == "__main__":
    ...
    y1, y2 = model_restoration(x)
    print(y1.shape, y2.shape)
    print("model params: %d" % count_parameters(model_restoration))
    # print("number of GFLOPs: %.2f G"%(model_restoration.flops() / 1e9))

# python -m model.uformer
