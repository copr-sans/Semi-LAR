...
class TrainLabeled(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # resize
        resize_size = self.fineSize + 64
        resized_a = A.resize((resize_size, resize_size), Image.LANCZOS)
        resized_b = B.resize((resize_size, resize_size), Image.LANCZOS)
        resized_c = C.resize((resize_size, resize_size), Image.LANCZOS)
...
def data_aug(images):
    ...
    color_jitter = transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.02)
    strong_aug = images
